optLL_CE_pre <- function(XAfreq, YAfreq, X_range, Y_range, A_range, max_power = 5, lrt_pvalue_threshold = 0.05) {
  ...
  for (XAmodel_name in names(best_XAmodels)) {
    ...
    for (YAmodel_name in names(best_YAmodels)) {
      ...
      keq_result <- kequate("NEAT_CE", X_range, Y_range, A_range, XAmodel, YAmodel)
      
      # Calculate average SEE
      avg_see <- norm(keq_result@PRE$PREAx + keq_result@PRE$PREYa, type="2") 
